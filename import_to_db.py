...
def import_file(year):
    ...
    # Check if table already has data to avoid duplicate import
    ...
    with open(filepath, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            # Skip header rows (row with "Statistic" in first column)
            if row[0].strip().lower() == "statistic":
                continue
            
            # Skip rows like '1876 N.L. History | Year-by-Year History'
            first_cell = row[0].strip()
            if first_cell and first_cell[0].isdigit() and "history" in first_cell.lower():
                continue

            # Skip malformed rows with less than 5 columns
            if len(row) < 5:
